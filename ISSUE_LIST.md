...
> **測定メモ (2026-08-14):** 今すぐ処理する active TODO は 0。運用ゲートは full tests / ops check / secret・path scan / open PR で別途判定する。下表の `parked` は次回注文候補、`blocked` は人間・live環境待ちであり、未実装をdone扱いしない。`v0.11.0` tag/Release は別承認。
>
Updated: 2026-08-14
...
| local WIP | ok | stash 0件。local-private cache backfill の旧試作前提は stale として閉じた |
...
| ID | 状態 | TODO | 完了条件 | 検証 |
|---|---|---|---|---|
| P0-1 | done | README / ROADMAP / 旧closeout docs の残務表現を揃える | README の残TODO、ROADMAP、過去closeout文書が「現在のactive TODOは ISSUE_LIST」と読める | README / ROADMAP の pointer と全旧 chat-context の historical banner を `test_docs_hierarchy.py` で固定 |
...
| P0-5 | done | stale closeout の「残務0」を誤読させない | `docs/2026-07-01-*` / 旧 chat-context は historical と明示され、active TODO と混ざらない | 対象文書すべての historical / ISSUE_LIST / ROADMAP pointer を `test_docs_hierarchy.py` で検証 |
...
| P1-2 | done | slow tests を短縮する | `test_discord_inventory_dashboard_omits_sensitive_values` など上位slow testsの待ち時間をfixture化・分離 | 2026-08-14: `pytest -q --durations=20` は 866 passed / 11.44秒、最上位0.93秒。対象2件は0.04秒 / 0.70秒 |
| P1-3 | parked | context parser quality を測る | `context-passport` / `guide-reply` の必須field充足率を fixture で測る | 次回の品質計測注文で metadata-only quality report を設計・実装 |
| P1-4 | parked | cache再利用率を測る | target_key一致時に再parse回避率が出る | 次回のcache注文で `cache_reuse_rate` metadata を設計・実装 |
| P1-5 | parked | read-more 誤停止を測る | blocker reason と人間判断結果を保存できる | 次回の判断ledger注文で sample ledger / fixture を設計・実装 |

## P2: 保守性と設計負債

| ID | 状態 | TODO | 完了条件 | 検証 |
|---|---|---|---|---|
| P2-1 | parked | `core.py` を責務ごとに分割する | URL intake、snapshot ledger、context/reply、send closeout が別moduleに分かれる | 次回のrefactor注文で public API互換を保つ別PRにする |
| P2-2 | parked | `test_core.py` をfeature別に分割する | slow / failure-localization が改善する | P2-1と同じrefactor注文で `pytest --durations=20` / CIを比較 |
| P2-3 | stale | local-private cache backfill を別設計にする | 旧stash試作は存在せず、対象のない作業をactiveにしない | 新しい要件とsourceが届いた時に別設計として再起票 |
| P2-4 | done | docsの階層を整理する | README =入口、ROADMAP =順序、ISSUE_LIST =active TODO、chat-context =履歴、full-reference =詳細 | `test_docs_hierarchy.py` で役割、履歴pointer、主要文書のinline Markdown linkを検証 |
...
現在activeは0。次に注文が入った時は、独立PRとして次から選ぶ。

1. P1-3: context parser quality を fixture で測る。
2. P1-4 / P1-5: cache reuse と read-more 人間判断を測定する。
3. P2-1 / P2-2: `core.py` と `test_core.py` を分割し、速度と見通しを上げる。
4. P3-1: live rehearsal は人間承認と実チャンネル準備後に別実行する。

## Stopline

- active TODO 0と将来roadmap完了を混同しない。`parked` / `later` / `blocked` は未実装のまま保持する。
