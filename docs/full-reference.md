...
候補数、ユーザー操作直前の最新対象 snapshot、copy block との一致、pre-send ping が揃らない場合は `blocked` になり、
...
```bash
python3 scripts/ops_preflight.py --app-name Discord --json
```
...
macOS Accessibility は既定で Discord を前面化しません。window 候補を確認した上で、
対象 window の full scan だけを試します。前面化して focused element も試す場合だけ `--focus-app` を明示します。
...
前面化が必要な場合だけ、自動fallback付きで opt-in します。これは focused element を短く試し、
だめなら同じ window の full scan、必要なら前面 window へ戻す順で確認します。

```bash
PYTHONPATH=src python3 scripts/live_ops_smoke.py \
  --source-command "python3 scripts/read_visible_discord_text.py --macos-accessibility-auto --focus-app --window-index 1 --timeout 3" \
  --source-timeout 12 \
  --channel visible-thread \
  --reset
```
