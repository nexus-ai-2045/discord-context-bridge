...
| P0 | 正本の範囲が曖昧 | 専用repo単体を正本にする前提なら解消済み。親repo側の状態は別lane。 | DCB closeoutでは `C:\Temp\dcb-user-example\Projects\Documents\discord-context-bridge` の `main` / `origin/main` 同期を正本条件にする。 |
