...
1. `schemas/` または `tests/fixtures/schemas/` に report / acquisition context の JSON schema を置く。
2. `source_kind` の taxonomy を単一 registry に寄せ、自由記述の source label と公開 report 用の分類を分ける。
3. legacy snapshot の normalization を追加し、`acquisition_context` 欠落時は `unknown_legacy_snapshot` として明示する。
4. private producer contract の fixture を追加し、producer が保存してよい metadata と保存してはいけない本文・path・credential を分ける。
5. `--include-preview` を developer verification として維持するか、public CLI から隠すかを人間レビューで決める。
