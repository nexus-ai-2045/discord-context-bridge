...
def _platform_default_user_data_dir(
    *, platform_name: str, env: Mapping[str, str], home: Path
) -> Path | None:
    if platform_name.startswith("win"):
...
def _read_simple_cache_key(path: Path) -> str:
    with path.open("rb") as handle:
        ...
        if len(header_data) != SIMPLE_HEADER.size:
            raise CacheEntryError("entry_too_short")
        initial_magic, version, key_length, _key_hash, _padding = SIMPLE_HEADER.unpack(header_data)
        if initial_magic != SIMPLE_INITIAL_MAGIC or version != SIMPLE_VERSION:
            raise UnsupportedCacheFormat("simple_cache_v5_required")
        if key_length < 1 or key_length > 1024 * 1024:
            raise CacheEntryError("invalid_key_length")
        key_data = handle.read(key_length)
        if len(key_data) != key_length:
            raise CacheEntryError("invalid_key_length")
    return key_data.decode("utf-8", errors="replace")
    ...
    *,
    ...
    for path in cache_root.iterdir():
        ...
        if counters["file_count"] >= max_files:
            scan_complete = False
            scan_stop_reason = "max_files_reached"
            break
        if time.monotonic() - scan_started >= max_scan_seconds:
            scan_complete = False
            scan_stop_reason = "max_scan_seconds_reached"
            break
        ...
        if guild_id not in key and channel_id not in key:
            continue
    ...
    result = _result_envelope(
        state=state,
        ok=state in {"metadata_found", "reference_only", "partial_scan"},
        platform_name=platform_value,
        source=resolved.source,
        user_data_dir_resolved=True,
    )
