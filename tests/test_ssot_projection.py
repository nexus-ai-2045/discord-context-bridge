...
def test_lint_ingest_route_policy_requires_playwright_default_ban(tmp_path):
    ...
    required = (
        ...
        "no_unapproved_visible_ui_automation no_browser_before_dcb_preflight "
        "内部ブラウザやChromeより先にDCB ingress ready_for_bridge "
        "ambient UIのDiscord URLだけを根拠にDCBを迂回しない "
        "no_visible_read_without_snapshot_closeout "
        "可視DOMを読んだ場合 bridge-intake snapshot.saved=true "
        "保存確認より先に要約 "
