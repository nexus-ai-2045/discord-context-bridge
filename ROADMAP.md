...
flowchart TD
  ...
  s4 --> s5["5 仮下書き"]
  s5 --> s6["6 risk review"]
  s6 --> s7["7 Markdown review artifact"]
  s7 --> s8["8 final candidate"]
  s8 --> s9["9 human gate"]
  s9 --> s10["10 copy block"]
  s10 --> s11["11 送信後観測"]
  s11 --> s12["12 Context SSOT 更新"]
  s12 --> s13["13 handoff"]
...
| 4 | 文脈理解チェック | 3-9要点で確定/推定/未確認を分ける | fixture understanding summary | raw 本文を出さない | artifact の理解欄になる |
| 5 | 仮下書きプローブ | 投稿候補ではない provisional draft を作る | draft probe fixture | do_not_post 表示 | risk review の入力になる |
| 6 | 下書きリスクレビュー | context/rule/tone/timing/missing premise を判定する | risk review fixture | send/reaction/delete 不可 | artifact のrisk欄になる |
| 7 | Markdown review artifact | 理解、draft、risk、unknown、選択肢を保存する | artifact生成 smoke | private data grep | E2Eの中心成果物 |
| 8 | 最終返信候補 | final candidate を作るが未送信にする | candidate fixture | 長さとtone確認 | human gate の入力になる |
| 9 | 人間判断 gate | copy/修正/追加読み/待機/返信なしを選べる | gate state fixture | 自動送信なし | E2Eで判断stateを残す |
| 10 | コピー可能な出力 | copy_block を短く返し、長文は分割/別形式へ逃がす | copy block fixture | 3分割以上は止める | E2Eで copy_block を残す |
| 11 | 送信後観測 | 依頼がある時だけ follow-up を観測する | follow-up metadata fixture | 自動追撃なし | E2Eでは optional state |
